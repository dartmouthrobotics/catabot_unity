...
namespace UnityEditor.Perception.GroundTruth
{
    ...
    class LabelingEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            m_EnableRendererToggle = m_Root.Q<Toggle>("enable-renderer");
            ...
            m_EnableRendererToggle.RegisterValueChangedCallback(evt =>
            {
                EnableRendererToggleChanged();
            });
        ...
        void EnableRendererToggleChanged() {
            UpdateUiAspects();

            foreach (var targetObj in serializedObject.targetObjects)
            {
                var serObj = new SerializedObject(targetObj);
                serObj.FindProperty(nameof(Labeling.enableRenderer)).boolValue = m_EnableRendererToggle.value;
                serObj.ApplyModifiedProperties();
                serObj.SetIsDifferentCacheDirty();
            }
        }
        ...
        void AssessEnableRenderer() {
            m_EnableRendererToggle.value = new SerializedObject(serializedObject.targetObjects[0]).FindProperty(nameof(Labeling.enableRenderer)).boolValue;
        }
